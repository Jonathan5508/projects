...
const InviteDB = require("../Structures/Schemas/InviteLogChannel")
...
client.on("guildMemberAdd", async member => {
    ...
    guild.invites.fetch().then(async newInvites => {
        ...
        const Data = await InviteDB.findOne({ Guild: guild.id }).catch(err => { })
        if (!Data) return
        ...
        if (!invite) return Channel.send({
            embeds: [
                Embed
                    .setDescription(`${user.tag} has joined, but the invite code is invalid.`)
            ]
        })
        ...
        djsClient.grabProfile(invite.inviter.id).then(async inviter => {
            ...
            return Channel.send({
                embeds: [
                    Embed.setDescription(`${user.tag} has joined using invite code \`${invite.code}\` from ${Inviter}. The code has been used a total of ${invite.uses} time(s).`)
                ]
            })
        }).catch(err => { })
    }).catch(err => {
